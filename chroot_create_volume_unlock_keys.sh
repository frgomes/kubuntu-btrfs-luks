...
function chroot_create_volume_unlock_keys() {
  ...
  echo "${passphrase}" | cryptsetup luksAddKey --key-file=- ${partition}2 /boot/volume-swap.key
  echo "${passphrase}" | cryptsetup luksAddKey --key-file=- ${partition}4 /boot/volume-root.key
